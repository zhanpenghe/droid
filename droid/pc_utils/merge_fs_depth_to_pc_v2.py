...
Outlier removal is done on the depth image (2D median filter) before
unprojection — much faster than 3D radius outlier removal on the merged cloud.

Usage:
  python merge_fs_depth_to_pc_v2.py [--capture_dir PATH] [--baseline 0.12]
...
if __name__ == "__main__":
    ...
    for cam_id in CAMERAS:
        ...
        n_removed = (np.isfinite(depth_raw) & (depth_raw > args.min_depth) & (depth_raw < args.max_depth)).sum() - len(pts)
    ...
    print(f"\n  Total processing time: {t_total*1000:.0f} ms  "
          f"(inference: {t_total*1000:.0f} ms breakdown above)")

    # ── Save (not timed) ────────────────────────────────────────────
    out_path = os.path.join(capture_dir, "pointcloud_fs_merged_v2.ply")
